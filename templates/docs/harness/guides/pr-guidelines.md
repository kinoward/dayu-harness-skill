...
- 长度至少 5 字符
- 建议使用自然语言描述性标题
- 项目采用自动化版本发布且使用 merge commit 策略时，标题不得使用 conventional commit 格式，以避免 changelog 重复条目；PR Lint 会拒绝 `feat:`、`fix:`、`docs:` 等 conventional commit 标题
...
- Test plan 执行：AI 执行规约存在时，按其中的 Test plan 规则逐项验证
- 合并策略：分支保护规约存在时，按其中的合并策略执行
