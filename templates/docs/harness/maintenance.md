...
凡 `AGENTS.md` 或 `README.md` 中存在 `## 目录结构` 区块，目录、文件、子目录或 capability 部署清单变化时，必须同步更新该区块。`AGENTS.md` 的目录结构只描述本层和直接子目录；`README.md` 可描述项目或部署后的整体结构。
...
- **目录结构同步**：索引、链接和 `## 目录结构` 区块必须同时维护，避免目录树与实际文件或已启用能力脱节
...
3. 用户确认后执行：
   - 删除文档文件
   - 删除关联联动组件
   - 更新所有受影响的 AGENTS.md 索引及相关 `## 目录结构` 区块（含 `README.md`）
   - 更新联动关系表
4. 验证：确认无断链残留
...
1. **AGENTS.md 链路**
   ...
   - `AGENTS.md` / `README.md` 中的 `## 目录结构` 区块是否与实际目录和已启用能力一致
...
2. 更新该目录的 `AGENTS.md`，添加文件条目，并同步该目录的 `## 目录结构` 区块
3. 如果是新建子目录，同时创建该子目录的 `AGENTS.md`，在父级 `AGENTS.md` 中添加子目录入口，并同步所有受影响的 `## 目录结构` 区块
