...
- `harness/`：治理约束、反馈检查和维护流程，是 AI 与人类协作时遵守的规则系统
- `design-docs/`：架构与设计决策记录（ADR），属于项目知识/经验
- `troubleshooting/`：可复用排障知识，属于项目知识/经验
- `references/`：外部资料与版本化研究，属于项目知识/经验
- `product-specs/`：产品规格和项目专属上下文，属于项目内容
- `exec-plans/`：任务执行计划和阶段进度
- `generated/`：自动生成资料的临时入口，确认后再沉淀到长期目录
- `archive/`：历史归档
